...
try:
    ...
    from orchestration.runtime_git import remove_worktree, GitRuntimeError
    ...
    from runtime_git import remove_worktree, GitRuntimeError
    ...
    def cmd_add(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if self.store.get_record(task_id) is not None:
                print(
                    f"Error: Record with id '{task_id}' already exists", file=sys.stderr
                )
                return 1

            # Get task body from --task or --task-file
            if args.task_file:
                # Load from file using normalize_task_from_file to handle frontmatter
                try:
                    canonical_path = normalize_task_from_file(
                        args.task_file, task_id, repo, base
                    )
                ...
                # Create canonical task markdown from inline task
                body = args.task
                try:
                    content = create_canonical_task_file(task_id, repo, body, base)
            ...
            # Create and add runtime record to queue
            frontmatter, _ = read_task_file(task_id)
            task_file_rel = str(Path("queue") / "tasks" / f"{task_id}.md")
...
def main():
    ...
    add_parser.add_argument("--repo", required=True, help="Repository path or name")
